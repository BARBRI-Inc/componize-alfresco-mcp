...
- **Phase 2 (not started)**: a Claude Enterprise custom connector (remote HTTP MCP, no per-user install at all). Confirmed the Componize instance is already reachable over the public internet but supports only individual basic-auth accounts — no OAuth/SSO of its own. Claude's custom connectors require the server to be public **and** OAuth-gated, so this means building our own OAuth-fronting layer (likely behind BARBRI SSO) plus an encrypted per-user credential store linking each person to their Componize login. That's real security-sensitive engineering deserving its own design/review, not a quick follow-on — see `../fabric-mcp` for the closest existing precedent (public HTTPS via Compass AKS + Front Door, per-user Entra On-Behalf-Of token exchange).
