    ...
    try:
        ...
        try:
            search_request = SearchRequest(
                query=RequestQuery(query=final_query, language=RequestQueryLanguage.AFTS),
            ...
            if error:
                logger.error(f"Content search failed: {error}")
                return safe_format_output(f"ERROR: Content search failed - {error}")
